...
const PokemonList = () => {
  ...
  const {isFetching, isLoading, resolvedData, isError} = usePaginatedQuery(
    ['pokemonList', current],
    getPokemonList,
  );
  ...
  return (
    <div className={blockName}>
      ...
      <div className={`${blockName}__footer`}>
        <button onClick={setCurrentPrevPage} disabled={!previous}>
          Previous
        </button>
        <button onClick={setCurrentNextPage} disabled={!next}>
          Next
        </button>
      </div>
    </div>
